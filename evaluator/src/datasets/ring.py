...
from configs.ring import Config as cfg
...
class Seg2Tunnel(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if cfg.flag_pipe == 'sphere_crop':
            ...
            actual_points = raw_pc.shape[0]
            if actual_points < cfg.num_points:
                # If ring is too small, query all available points
                dist, neigh_idx = search_tree.query([centre[0:3]], k=actual_points)
                dist = dist[0]
                neigh_idx = list(neigh_idx[0])
                
                # Randomly duplicate points to reach the required num_points
                padding_needed = cfg.num_points - actual_points
                padding_idx = np.random.choice(neigh_idx, padding_needed, replace=True)
                neigh_idx.extend(padding_idx)
                
                dist = np.pad(dist, (0, padding_needed), mode='edge')
                neigh_idx = np.asarray(neigh_idx)
            else:
                # Normal query for large enough rings
            ...
            # Use np.add.at to safely handle the duplicated indices during tracking
            np.add.at(self.possibility[station], neigh_idx, delta)
