...
# -----------------------------------------------------------------------------
# NEW: Depth Unprojection Function
# -----------------------------------------------------------------------------
def depth_to_pointcloud(img, disp, acc, K, c2w, acc_threshold=0.99):
    """
    Converts a NeRF 2D render and disparity map into a 3D point cloud,
    using the accumulated opacity (acc) map to aggressively filter noise.
    """
    # 1. Convert to NumPy
    if torch.is_tensor(img): img = img.detach().cpu().numpy()
    if torch.is_tensor(disp): disp = disp.detach().cpu().numpy()
    if torch.is_tensor(acc): acc = acc.detach().cpu().numpy()
    if torch.is_tensor(c2w): c2w = c2w.detach().cpu().numpy()
    ...
    # 2. THE MAGIC FIX: Filter by opacity (acc) instead of just disparity!
    # This deletes the invisible "fog" floating in the background.
    valid_mask = acc > acc_threshold
    ...
    colors_valid = img[valid_mask]

    # 4. Project to Camera Space 
    x_cam = (u_valid - cx) * z_valid / f
    y_cam = -(v_valid - cy) * z_valid / f # (Includes the Y-axis flip fix!)
    z_cam = -z_valid
    ...
    for i, (entry, cat, obj_path) in enumerate(dataloader):
        ...
        for j, target_w in enumerate(nerf_Ws):
            ...
            with torch.no_grad():
                ...
                # NEW: Initialize lists to collect points and colors from every frame
                all_points = []
                all_colors = []

                for k, pose in enumerate(render_poses):

                    # 1. Render the image (happens every frame so the GIF stays smooth)
                    ...
                    # --- NEW: REDUCE CAMERA OVERLAP ---
                    # 2. Only extract points on every 5th frame to prevent thickness stacking
                    if k % 3 == 0:
                        # Note: acc_threshold is set to 0.95 here, which is a great sweet spot
                        # when combined with the reduced camera overlap!
                        pts, colors = depth_to_pointcloud(img, disp, acc, K, pose, acc_threshold=0.95)
                    ...
                    # 3. Save static images occasionally
                    if k % 4 == 0:
                        imageio.imsave(join(results_dir, f'o_{sample_id}_{k}.png'), to8b(img.detach().cpu().numpy()))

                # NEW: Save the final, dense point cloud to a .ply file
                if len(all_points) > 0:
                    ...
                    pc = trimesh.PointCloud(final_points, colors=final_colors)
                    pc_path = join(results_dir, f'o_high_detail_pointcloud_{sample_id}.ply')
                    pc.export(pc_path)
                    print(f"Saved high-detail point cloud ({len(final_points)} points) to {pc_path}")
