...
def save_alignment_visualization(pred_pts, gt_rotated, gt_orig, save_path):
    """Saves a 3D scatter plot of the alignment for debugging."""
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111, projection='3d')
    
    # Plot each point cloud. s=0.5 sets the point size.
    ax.scatter(pred_pts[:, 0], pred_pts[:, 1], pred_pts[:, 2], c='r', s=0.5, label='Pred (Red)')
    ax.scatter(gt_orig[:, 0], gt_orig[:, 1], gt_orig[:, 2], c='g', s=0.5, label='GT Orig (Green)')
    
    ax.legend()
    ax.set_title("Alignment Visualization")
    
    # Save the figure and close it to prevent memory leaks in the loop
...
def calculate_mesh_metrics(obj_path, render_kwargs, save_pc=True, name="1", threshold = 3):
    ...
    # Look in the folder from your screenshot
    full_dir_path = os.path.join(config['data_dir'], "seg2tunnel", "sampled", "eval")
    txt_path = os.path.join(full_dir_path, base_name + ".txt")
    ply_path = os.path.join(full_dir_path, base_name + ".ply")

    NUM_SAMPLES = 30000 
    
    # --- ADDED: PROGRESS MONITORING ---
    print(f"\n[GEOMETRY EVAL] Starting evaluation for: {base_name}")
    
    try:
        # --- 2. LOAD PRE-GENERATED PREDICTION (.PLY) ---
        if not os.path.exists(ply_path):
            print(f"  -> [WARNING] Predicted mesh not found: {ply_path}. Skipping.")
            return torch.Tensor([0.0]), torch.Tensor([0.0])
            
        print(f"  -> Loading prediction: {base_name}.ply")
        pred_mesh = trimesh.load_mesh(ply_path, force='mesh')
        pred_mesh = as_mesh(pred_mesh)
        
        # Sample points from your pre-generated mesh
        sampled_points = trimesh.sample.sample_surface(pred_mesh, NUM_SAMPLES)
        sampled_points = torch.from_numpy(sampled_points[0])[None].to(device, dtype=torch.float)


        # --- 3. LOAD GROUND TRUTH (.TXT) ---
        if not os.path.exists(txt_path):
            print(f"  -> [WARNING] GT file not found: {txt_path}. Skipping.")
            return torch.Tensor([0.0]), torch.Tensor([0.0])

        print(f"  -> Loading ground truth: {base_name}.txt")
        gt_data = np.loadtxt(txt_path) 
        if gt_data.size == 0:
            print(f"  -> [WARNING] GT file {base_name}.txt is empty. Skipping.")
            return torch.Tensor([0.0]), torch.Tensor([0.0])
        if gt_data.ndim == 1:
        ...
        if gt_xyz.shape[0] >= NUM_SAMPLES:
            idx = np.random.choice(gt_xyz.shape[0], NUM_SAMPLES, replace=False)
            entry_points_np = gt_xyz[idx, :]
        else:
            idx = np.random.choice(gt_xyz.shape[0], NUM_SAMPLES, replace=True)
            entry_points_np = gt_xyz[idx, :]

        entry_points = torch.from_numpy(entry_points_np).to(device, dtype=torch.float)

        # ---------------------------------------------------------
        # --- 1. PREPARE THE VISUALIZATION DATA ---
        # We calculate the rotated version ONLY for the debug plot
        entry_points_rotated = rot_x(math.pi/2).to(device) @ entry_points.T
        entry_points_rotated = entry_points_rotated.T
        
        # Define where to save the image
        vis_save_path = os.path.join(config['results_dir'], "eval", f"{base_name}_alignment.png")
        
        # Convert tensors back to numpy for plotting
        pred_np = sampled_points[0].detach().cpu().numpy()
        gt_rot_np = entry_points_rotated.detach().cpu().numpy()
        gt_orig_np = entry_points_np # Already numpy
        
        print(f"  -> Saving alignment visualization to {vis_save_path}")
        save_alignment_visualization(pred_np, gt_rot_np, gt_orig_np, vis_save_path)
        # ---------------------------------------------------------


        # ---------------------------------------------------------
        # --- 2. CALCULATE THE ACTUAL METRICS (THE FIX) ---
        # We use the ORIGINAL, unrotated entry_points for the real metrics!
        entry_points = entry_points[None] 

        print(f"  -> Calculating Chamfer Distance and F-Score...")
        dist1, dist2, idx1, idx2 = chfp.distChamfer(entry_points, sampled_points)
        cd = (torch.mean(dist1)) + (torch.mean(dist2))
        f_score, precision, recall = fscore(dist1, dist2, 0.01)
        # ---------------------------------------------------------

        print(f"  -> [SUCCESS] Finished {base_name}. CD: {cd.item():.4f}, F-score: {f_score.item():.4f}")
        return f_score, cd 
...
if __name__ == '__main__':
    ...
    # 1. Save the raw metrics for every single sample
    # This creates a big table with every file's individual F-score, CD, and PSNR
    eval_results.to_csv(join(results_dir, "metrics.csv"), index=False)
    
    # 2. Save the summary statistics grouped by class (Mean, Std, Min, Max, etc.)
    grouped = eval_results.groupby("class").describe()
    grouped.to_csv(join(results_dir, "metrics_summary_by_class.csv"))
    
    # 3. Save the overall summary statistics for the whole dataset
    overall = eval_results[['fscore', 'cd', 'psnr']].describe()
    overall.to_csv(join(results_dir, "metrics_overall.csv"))
    
    print(f"Saved metrics successfully to: {results_dir}")